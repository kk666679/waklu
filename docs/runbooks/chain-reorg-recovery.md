...
## Symptom
The indexer detects a reorg: a row in `ProductsOnChain` / `CertificatesOnChain` / etc. has `blockHash` (stored in Phase 3) that no longer matches the canonical chain at that block height.
